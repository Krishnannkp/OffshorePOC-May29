...
var paths = {
	tscripts: {
		src: ["typings/tsd.d.ts", "app/app.consts.ts", "app/app.ts", "app/**/*.ts", "app/**/*.*.ts", "!./**/*.spec.ts"],
		dest: "app/build"
	},
	testscripts: {
		src: ["app/**/*.spec.ts"],
...
gulp.task("build", ["compile:typescript", "compile:css", "bower", "html", "copy:fonts", "copy:imgs"]);

gulp.task("testbuild", ["compile:testtypecript"]);
...
gulp.task("compile:testtypecript", function () {
	var tsResult = gulp
		.src(paths.testscripts.src)
		.pipe(sourcemaps.init())
		.pipe(tsc())
		.pipe(sourcemaps.write("."))
		.pipe(gulp.dest("./build/testscripts"));
});
